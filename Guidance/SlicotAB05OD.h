...
#ifndef __SLICOT_AB05OD_H__
#define __SLICOT_AB05OD_H__

// File granularity version.
#ifndef SLICOT_AB05OD_VERSION_MAJOR
#define SLICOT_AB05OD_VERSION_MAJOR 1
#endif

#ifndef SLICOT_AB05OD_VERSION_MINOR
#define SLICOT_AB05OD_VERSION_MINOR 0
#endif

#ifndef SLICOT_AB05OD_PATCH_VERSION
#define SLICOT_AB05OD_PATCH_VERSION 0
#endif

#ifndef SLICOT_AB05OD_CREATE_DATE
#define SLICOT_AB05OD_CREATE_DATE "Date: 05-10-2016 Time: 15:47 PM GMT+2"
#endif

// Set this value to successful build date/time.
#ifndef SLICOT_AB05OD_BUILD_DATE
#define SLICOT_AB05OD_BUILD_DATE ""
#endif

#ifndef SLICOT_AB05OD_AUTHOR
#define SLICOT_AB05OD_AUTHOR "Name: Bernard Gingold , e-mail: beniekg@gmail.com"
#endif
...
namespace guidance {
	namespace slicot_wrappers {
		...
		template<typename R64 = double,
				 typename I32 = int,
				 typename UC8 = unsigned char>  struct  F77_AB05OD {
					 ...
					 */
					 ...
					 /*
					   @Purpose:
									...
									routine.
					 */
					 F77_AB05OD(_In_ const UC8 OVER,
							    _In_ const I32 N1,
						        _In_ const I32 M1,
						        _In_ const I32 P1,
						        _In_ const I32 N2,
						 ...
						 m_LDB2{ LDB2 },
						 m_B2{ new R64[m_LDB2 * m_M2] },
						 m_LDC2{ LDC2 },
						 m_C2{ new R64[m_LDC2 * m_N2] },
						 m_LDD2{ LDD2 },
						 m_D2{ new R64[m_LDD2 * m_M2] },
						 m_N{ N },
						 m_M{ M },
						 m_LDA{ LDA },
						 m_A{ new R64[m_LDA * (m_N1 + m_N2)] },
						 m_LDB{ LDB },
						 m_B{ new R64[m_LDB * (m_M1 + m_M2)] },
						 m_LDC{ LDC },
						 m_C{ new R64[m_LDC * (m_N1 + m_N2)] },
						 m_LDD{ LDD },
						 m_D{ new R64[m_LDD * (m_M1 + m_M2)] },
						 m_INFO{ INFO } {
...
#pragma unroll(4)
							 ...
							 std::cerr << __DATE__ << ":" << __TIME__ << " FATAL ERROR: Invalid pointer in 1st Constructor of F77_AB05OD !!\n";
...
#pragma unroll(4)
#endif
								 for (int j{ 0 }; j != (this->m_M1 + this->m_M2); ++j) {
...
#pragma unroll(4)
					 ...
					 std::cerr << "[" << __DATE__ << ":" << __TIME__ << "]" << " FATAL ERROR: Invalid pointer in 2nd Ctor of F77_AB05OD !!\n";
					 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
					 std::cerr << "**** ERROR DETAILS ****\n";
...
#pragma unroll(4)
					 ...
					 std::cerr << __DATE__ << ":" << __TIME__ << " FATAL ERROR: Invalid pointer in 2nd Constructor of F77_AB05OD !!\n";
					 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
					 std::cerr << "**** ERROR DETAILS ****\n";
					 std::cerr << "Value of m_A=" << std::hex << "0x" << this->m_A << "\n";
					 std::cerr << "Cannot recover -- calling std::exit(-1)!!\n";
...
#pragma unroll(4)
					...
					*/
		F77_AB05OD(_In_ const F77_AB05OD &other)
				:
			m_OVER{ other.m_OVER },
			m_N1{ other.m_N1 },
			m_M1{ other.m_M1 },
...
#endif
					for (int j{ 0 }; j != this->m_N1; ++j) {
						this->m_A1[i + this->m_LDA1 * j] = other.m_A1[i + other.m_LDA1 * j];
					}
				}
			}
			else {
				std::cerr << "[" << __DATE__ << ":" << __TIME__ << "]" << " FATAL ERROR: Invalid pointer in Copy Ctor of F77_AB05OD !!\n";
				std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
				std::cerr << "**** ERROR DETAILS ****\n";
...
#endif
					for (int j{ 0 }; j != this->m_N2; ++j) {
						this->m_C[i + this->m_LDC2 * j] = other.m_C2[i + other.m_LDC2 * j];
					}
				}
			}
			else {
...
#pragma unroll(4)
...
#endif
					for (int j{ 0 }; j != (this->m_M1 + this->m_M2); ++j) {
						this->m_D[i + this->m_LDD * j] = other.m_D[i + other.m_LDD * j];
					}
				}
			}
			else {
				...
				std::exit(-1);
					 ...
					 /*
					   @Purpose:
									Copy Constructor implements shallow copy semantics.
					 */
					 F77_AB05OD(_In_ F77_AB05OD &&other)
			              :
			           m_OVER{ other.m_OVER },
			           m_N1{ other.m_N1 },
			           m_M1{ other.m_M1 },
					 ...
					 */
					 F77_AB05OD & operator=(_In_ const F77_AB05OD &other) {
...
#pragma unroll(4)
							 ...
							 std::cerr << "[" << __DATE__ << ":" << __TIME__ << "]" << " FATAL ERROR: Invalid pointer in Copy Operator of F77_AB05OD !!\n";
							 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
							 std::cerr << "**** ERROR DETAILS ****\n";
						 ...
						 R64* __restrict tB1 = new R64[this->m_LDB1 * this->m_M1];
						 if (tB1) {
							 for (int i{ 0 }; i != this->m_LDB1; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_B1:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tB2 = new R64[this->m_LDB2 * this->m_M2];
						 if (tB2) {
							 for (int i{ 0 }; i != this->m_LDB2; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_B2:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tD2 = new R64[this->m_LDD2 * this->m_M2];
						 if (tD2) {
							 for (int i{ 0 }; i != this->m_LDD2; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_D2:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tB = new R64[this->m_LDB * (this->m_M1 + this->m_M2)];
						 if (tB) {
							 for (int i{ 0 }; i != this->m_LDB; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_B:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tD = new R64[this->m_LDD * (this->m_M1 + this->m_M2)];
						 if (tD) {
							 for (int i{ 0 }; i != this->m_LDD; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_D:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
					 ...
					 */
					 F77_AB05OD & operator=(_In_ F77_AB05OD &&other) {
						 if (this == &other) return (*this);
					 ...
					 /*
					   @Purpose:
								 Calls Fortran AB05OD subroutine.
					 */
					 void  call_AB05OD() {

						 AB05OD(&this->m_OVER,&this->m_N1,&this->m_M1,&this->m_P1,&this->m_N2,&this->m_M2,&this->m_ALPHA,
							 ...
							 &this->m_C2[0], &this->m_LDC2, &this->m_D2[0], &this->m_LDD2, &this->m_N, &this->m_M,
							 &this->m_A[0], &this->m_LDA, &this->m_B[0], &this->m_LDB, &this->m_C[0], &this->m_LDC,
							 &this->m_D[0], &this->m_LDD, &this->m_INFO);
					 }
