...
#ifndef __SLICOT_AB05ND_H__
#define __SLICOT_AB05ND_H__

// File version granularity.
#ifndef SLICOT_AB05ND_VERSION_MAJOR
#define SLICOT_AB05ND_VERSION_MAJOR 1
#endif

#ifndef SLICOT_AB05ND_VERSION_MINOR
#define SLICOT_AB05ND_VERSION_MINOR 0
#endif

#ifndef SLICOT_AB05ND_PATCH_VERSION
#define SLICOT_AB05ND_PATCH_VERSION 0
#endif

#ifndef SLICOT_AB05ND_CREATE_DATE
#define SLICOT_AB05ND_CREATE_DATE "Date: 03-10-2016  Time: 15:07 PM GMT+2"
#endif

// Set this value to successful build date/time.
#ifndef SLICOT_AB05ND_BUILD_DATE
#define SLICOT_AB05ND_BUILD_DATE ""
#endif

#ifndef SLICOT_AB05ND_AUTHOR
#define SLICOT_AB05ND_AUTHOR "Name: Bernard Gingold ,  e-mail: beniekg@gmail.com"
#endif
...
namespace guidance {
	namespace slicot_wrappers {
		...
		template<typename R64 = double,
				 typename I32 = int,
				 typename UC8 = unsigned char>  struct F77_AB05ND {
					 ...
					 */
					 ...
					 /*
						@Purpose:
									...
									routine.
					 */
					 F77_AB05ND(_In_ const UC8 OVER,
							    _In_ const I32 N1,
						        _In_ const I32 M1,
						        _In_ const I32 P1,
						        _In_ const I32 N2,
...
#pragma unroll(4)
							 ...
							 std::cerr << __DATE__ << ":" << __TIME__ << " FATAL ERROR: Invalid pointer in 1st Constructor of F77_AB05ND !!\n";
...
#pragma unroll(4)
						 ...
						 if (this->m_IWORK) {
							 ...
							 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
							 std::cerr << "**** ERROR DETAILS ****\n";
							 std::cerr << "Value of m_IWORK=" << std::hex << "0x" << this->m_IWORK << "\n";
							 std::cerr << "Cannot recover -- calling std::exit(-1)!!\n";
							 std::exit(-1);
						 }
						 if (this->m_DWORK) {
...
#pragma unroll(4)
				 ...
				 std::cerr << "[" << __DATE__ << ":" << __TIME__ << "]" << " FATAL ERROR: Invalid pointer in 2nd Ctor of F77_AB05ND !!\n";
				 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
				 std::cerr << "**** ERROR DETAILS ****\n";
...
#pragma unroll(4)
				 ...
				 std::cerr << __DATE__ << ":" << __TIME__ << " FATAL ERROR: Invalid pointer in 2nd Constructor of F77_AB05ND !!\n";
				 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
				 std::cerr << "**** ERROR DETAILS ****\n";
				 std::cerr << "Value of m_A=" << std::hex << "0x" << this->m_A << "\n";
				 std::cerr << "Cannot recover -- calling std::exit(-1)!!\n";
...
#pragma unroll(4)
			 ...
			 if (this->m_DWORK) {
				 ...
				 std::exit(-1);
					 ...
					 */
		 F77_AB05ND(_In_ const F77_AB05ND &other)
			 :
			 m_OVER{ other.m_OVER },
			 m_N1{ other.m_N1 },
			 m_M1{ other.m_M1 },
...
#pragma unroll(4)
				 ...
				 std::cerr << "[" << __DATE__ << ":" << __TIME__ << "]" << " FATAL ERROR: Invalid pointer in Copy Ctor of F77_AB05ND !!\n";
				 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
				 std::cerr << "**** ERROR DETAILS ****\n";
...
#pragma unroll(4)
			 ...
			 if (this->m_IWORK) {
				 ...
				 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
				 std::cerr << "**** ERROR DETAILS ****\n";
				 std::cerr << "Value of m_IWORK=" << std::hex << "0x" << m_IWORK << "\n";
				 std::cerr << "Cannot recover -- calling std::exit(-1)!!\n";
				 std::exit(-1);
			 }
			 if (this->m_DWORK) {
...
#pragma unroll(4)
				 ...
				 std::exit(-1);
					 ...
					 */
		 F77_AB05ND(_In_ F77_AB05ND &&other)
			 :
			 m_OVER{ other.m_OVER },
			 m_N1{ other.m_N1 },
			 m_M1{ other.m_M1 },
			 ...
			 m_INFO{ other.m_INFO } {
				...
				other.m_DWORK = nullptr;
					 ...
					 */
					 F77_AB05ND & operator=(_In_ const F77_AB05ND &other) {
...
#pragma unroll(4)
							 ...
							 std::cerr << "[" << __DATE__ << ":" << __TIME__ << "]" << " FATAL ERROR: Invalid pointer in Copy Operator of F77_AB05ND !!\n";
							 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
							 std::cerr << "**** ERROR DETAILS ****\n";
...
#pragma unroll(4)
						 ...
						 I32* __restrict tIWORK = new I32[this->m_P1];
						 if (tIWORK) {
							 for (int i{ 0 }; i != this->m_P1; ++i) {
							 ...
							 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
							 std::cerr << "**** ERROR DETAILS ****\n";
							 std::cerr << "Value of tIWORK=" << std::hex << "0x" << tIWORK << "\n";
							 std::cerr << "Cannot recover -- calling std::exit(-1)!!\n";
							 std::exit(-1);
						 }
...
#pragma unroll(4)
					 ...
					 */
					  F77_AB05ND & operator=(_In_ F77_AB05ND &&other) {
						  if (this == &other) return (*this);
					  ...
					  /*
					    @Purpose:
									Calls Fortran AB05ND subroutine.
					  */
					  void  call_AB05ND() {
						  AB05ND(&this->m_OVER,&this->m_N1,&this->m_M1,&this->m_P1,&this->m_N2,&this->m_ALPHA,
							  &this->m_A[0], &this->m_LDA1, &this->m_B1[0], &this->m_LDB1, &this->m_C1[0],&this->m_LDC1,
							  &this->m_D1[0], &this->m_LDD1, &this->m_A2[0], &this->m_LDA2, &this->m_B2[0],&this->m_LDB2,
							  &this->m_C2[0], &this->m_LDC2, &this->m_D2[0], &this->m_LDD2, &this->m_N, &this->m_A[0],
							  &this->m_LDA, &this->m_B[0], &this->m_LDB, &this->m_C[0], &this->m_LDC, &this->m_D[0],&this->m_LDD,
							  &this->m_IWORK[0], &this->m_DWORK[0],&this->m_LDWORK,&this->m_INFO);
					  }
