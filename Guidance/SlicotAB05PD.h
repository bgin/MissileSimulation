...
#ifndef __SLICOT_AB05PD_H__
#define __SLICOT_AB05PD_H__

// File granularity version.
#ifndef SLICOT_AB05PD_VERSION_MAJOR
#define SLICOT_AB05PD_VERSION_MAJOR 1
#endif

#ifndef SLICOT_AB05PD_VERSION_MINOR
#define SLICOT_AB05PD_VERSION_MINOR 0
#endif

#ifndef SLICOT_AB05PD_PATCH_VERSION
#define SLICOT_AB05PD_PATCH_VERSION 0
#endif

#ifndef SLICOT_AB05PD_CREATE_DATE
#define SLICOT_AB05PD_CREATE_DATE "Date: 09-10-2016  Time: 11:21 AM GMT+2"
#endif

// Set this value to successful build date/time.
#ifndef SLICOT_AB05PD_BUILD_DATE
#define SLICOT_AB05PD_BUILD_DATE ""
#endif

#ifndef SLICOT_AB05PD_AUTHOR
#define SLICOT_AB05PD_AUTHOR "Name: Bernard Gingold , e-mail: beniekg@gmail.com"
#endif
...
namespace guidance {
	namespace slicot_wrappers {
		...
		template<typename R64 = double,
				 typename I32 = int,
				 typename UC8 = unsigned char>  struct  F77_AB05PD {
					 ...
					 */
					 ...
					 /*
					   @Purpose:
								 	1st "main" class Constructor.responsible for
									...
									routine.
					 */
					 F77_AB05PD(_In_ const UC8 OVER,
								_In_ const I32 N1,
						        _In_ const I32 M,
						        _In_ const I32 P,
						        _In_ const I32 N2,
						        _In_ const R64 ALPHA,
						        _In_ const I32 LDA1,
						        _In_ const I32 LDB1,
						        _In_ const I32 LDC1,
						        _In_ const I32 LDD1,
						 ...
						 m_LDB1{ LDB1 },
						 m_B1{ new R64[m_LDB1 * m_M] },
						 m_LDC1{ LDC1 },
						 m_C1{ new R64[m_LDC1 * m_N1] },
						 m_LDD1{ LDD1 },
						 m_D1{ new R64[m_LDD1 * m_M] },
						 m_LDA2{ LDA2 },
						 m_A2{ new R64[m_LDA2 * m_N2] },
						 m_LDB2{ LDB2 },
						 m_B2{ new R64[m_LDB2 * m_M] },
						 m_LDC2{ LDC2 },
						 m_C2{ new R64[m_LDC2 * m_N] },
						 m_LDD2{ LDD2 },
						 m_D2{ new R64[m_LDD2 * m_M] },
						 m_N{ N },
						 m_LDA{ LDA },
...
#pragma unroll(4)
							 ...
							 std::cerr << __DATE__ << ":" << __TIME__ << " FATAL ERROR: Invalid pointer in 1st Constructor of F77_AB05PD !!\n";
...
#pragma unroll(4)
				 ...
				 std::cerr << "[" << __DATE__ << ":" << __TIME__ << "]" << " FATAL ERROR: Invalid pointer in 2nd Ctor of F77_AB05PD !!\n";
				 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
				 std::cerr << "**** ERROR DETAILS ****\n";
...
#pragma unroll(4)
				 ...
				 std::cerr << __DATE__ << ":" << __TIME__ << " FATAL ERROR: Invalid pointer in 2nd Constructor of F77_AB05PD !!\n";
				 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
				 std::cerr << "**** ERROR DETAILS ****\n";
				 std::cerr << "Value of m_A=" << std::hex << "0x" << this->m_A << "\n";
				 std::cerr << "Cannot recover -- calling std::exit(-1)!!\n";
...
#pragma unroll(4)
				 ...
				 std::exit(-1);
					 ...
					 */
					 F77_AB05PD(_In_ const F77_AB05PD &other)
		                     :
		                m_OVER{ other.m_OVER },
		                m_N1{ other.m_N1 },
		                m_M{ other.m_M },
...
#endif
								 for (int j{ 0 }; j != this->m_N1; ++j) {
									 this->m_LDA1[i + this->m_LDA1 * j] = other.m_A1[i + other.m_LDA1 * j];
								 }
							 }
						 }
						 else {
							 std::cerr << "[" << __DATE__ << ":" << __TIME__ << "]" << " FATAL ERROR: Invalid pointer in Copy Ctor of F77_AB05PD !!\n";
							 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
							 std::cerr << "**** ERROR DETAILS ****\n";
...
#endif
								 for (int j{ 0 }; j != this->m_M; ++j) {
									 this->m_B1[i + this->m_LDB1 * j] = other.m_B1[i + other.m_LDB1 * j];
								 }
							 }
						 }
						 else {
...
#endif
								 for (int j{ 0 }; j != this->m_N1; ++j) {
									 this->m_C1[i + this->m_LDC1 * j] = other.m_C1[i + other.m_LDC1 * j];
								 }
							 }
						 }
						 else {
...
#endif
								 for (int j{ 0 }; j != this->m_M; ++j) {
									 this->m_D1[i + this->m_LDD1 * j] = other.m_D1[i + other.m_LDD1 * j];
								 }
							 }
						 }
						 else {
...
#endif
								 for (int j{ 0 }; j != this->m_N2; ++j) {
									 this->m_A2[i + this->m_LDA2 * j] = other.m_A2[i + other.m_LDA2 * j];
								 }
							 }
						 }
						 else {
...
#endif
								 for (int j{ 0 }; j != this->m_M; ++j) {
									 this->m_B2[i + this->m_LDB2 * j] = other.m_B2[i + other.m_LDB2 * j];
								 }
							 }
						 }
						 else {
...
#endif
								 for (int j{ 0 }; j != this->m_N2; ++j) {
									 this->m_C2[i + this->m_LDC2 * j] = other.m_C2[i + other.m_LDC2 * j];
								 }
							 }
						 }
						 else {
...
#endif
								 for (int j{ 0 }; j != this->m_M; ++j) {
									 this->m_D2[i + this->m_LDD2 * j] = other.m_D2[i + other.m_LDD2 * j];
								 }
							 }
						 }
						 else {
...
#endif
								 for (int j{ 0 }; j != (this->m_N1 + this->m_N2); ++j) {
									 this->m_C[i + this->m_LDC * j] = other.m_C[i + other.m_LDC * j];
								 }
							 }
						 }
						 else {
...
#pragma unroll(4)
					 ...
					 /*
					   @Purpose:
					 */
					 F77_AB05PD(_In_ F77_AB05PD &other)
		                      :
		                  m_OVER{ other.m_OVER },
		                  m_N1{ other.m_N1 },
		                  m_P{ other.m_P },
					 ...
					 */
					 F77_AB05PD & operator=(_In_ const F77_AB05PD &other) {
						 if (this == &other) return (*this);

						 this->m_OVER = other.m_OVER;
						 ...
						 R64* __restrict tA1 = new R64[m_LDA1 * m_N1];
						 if (tA1) {
							 for (int i{ 0 }; i != this->m_LDA1; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_A1:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
							 ...
							 std::cerr << "[" << __DATE__ << ":" << __TIME__ << "]" << " FATAL ERROR: Invalid pointer in Copy Operator of F77_AB05PD !!\n";
							 std::cerr << "at " << __FILE__ << ":" << __LINE__ << "(" << std::hex << "0x" << __FUNCTIONW__ << ")" << "\n";
							 std::cerr << "**** ERROR DETAILS ****\n";
						 ...
						 R64* __restrict tB1 = new R64[m_LDB1 * m_M];
						 if (tB1) {
							 for (int i{ 0 }; i != this->m_LDB1; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_B1:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tC1 = new R64[m_LDC1 * m_N1];
						 if (tC1) {
							 for (int i{ 0 }; i != this->m_LDC1; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_C1:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
...
#endif
								 for (int j{ 0 }; j != this->m_N1; ++j) {
									tC1[i + this->m_LDC1 * j] = other.m_C1[i + this->m_LDC1 * j];
								 }
							 }
							 delete[] this->m_C1;
							 this->m_C1 = tC1;
						 ...
						 R64* __restrict tD1 = new R64[m_LDD1 * m_M];
						 if (tD1) {
							 for (int i{ 0 }; i != this->m_LDD1; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_D1:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tA2 = new R64[m_LDA2 * m_N2];
						 if (tA2) {
							 for (int i{ 0 }; i != this->m_LDA2; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_A2:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tB2 = new R64[m_LDB2 * m_M];
						 if (tB2) {
							 for (int i{ 0 }; i != this->m_LDB2; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_B2:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tC2 = new R64[m_LDC2 * m_N2];
						 if (tC2) {
							 for (int i{ 0 }; i != this->m_LDC2; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_C2:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tD2 = new R64[m_LDD2 * m_M];
						 if (tD2) {
							 for (int i{ 0 }; i != this->m_LDD2; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_D2:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tA = new R64[m_LDA * (m_N1 + m_N2)];
						 if (tA) {
							 for (int i{ 0 }; i != this->m_LDA; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_A:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tB = new R64[m_LDB * m_M];
						 if (tB) {
							 for (int i{ 0 }; i != this->m_LDB; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_B:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tC = new R64[m_LDC * (m_N1 + m_N2)];
						 if (tC) {
							 for (int i{ 0 }; i != this->m_LDC; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_C:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
						 ...
						 R64* __restrict tD = new R64[m_LDD * m_M];
						 if (tD) {
							 for (int i{ 0 }; i != this->m_LDD; ++i) {
#ifdef ICC_PREFETCH_L1
#pragma prefetch other.m_D:0:L1_VEC_ITERS_SHORT
...
#pragma unroll(4)
					 ...
					 */
					 F77_AB05PD & operator=(_In_ F77_AB05PD &&other) {
						 if (this == &other) return (*this);
					 ...
					 /*
					   @Purpose:
					             Calls Fortran AB05PD subroutine.
					 */
					 void  call_AB05PD() {
						 AB05PD(&this->m_OVER, &this->m_A, &this->m_M, &this->m_P, &this->m_N2, &this->m_ALPHA, &this->m_A1[0],
							 &this->m_LDA1, &this->m_B1[0], &this->m_LDB1, &this->m_C1[0], &this->m_LDC1, &this->m_D1[0],
							 &this->m_LDD1, &this->m_A2[0], &this->m_LDA2, &this->m_B2[0], &this->m_LDB2, &this->m_C2[0],
							 &this->m_LDC2, &this->m_D2[0], &this->m_LDD2, &this->m_N,  &this->m_A[0], &this->m_LDA,
							 &this->m_B[0], &this->m_LDB, &this->m_C[0], &this->m_LDC, &this->m_D[0], &this->m_LDD, &this->m_INFO);
					 }
