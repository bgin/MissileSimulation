...
C             LDWORK >= c*N**2 + N + MAX(2*N, 24), where
C                       c = 18, if                 COMPU = 'C';
...
C
C     REVISIONS
C     V. Sima, Jan. 2011, Mar. 2011, Aug. 2011, Nov. 2011.
C     M. Voigt, Jan. 2012.
C
C     KEYWORDS
C
...
C     .. Intrinsic Functions ..
      INTRINSIC          ABS, CONJG, DBLE, DCMPLX, DIMAG, INT, MAX, MIN,
     $                   MOD, SQRT
C
...
C                        w2 = 6*N**2+MAX(2*N, 24),
C                                        if COMPQ = 'C' or  COMPU = 'C';
C                        w2 = 3*N**2+MAX(2*N, 24),
