...
C     accumulated in the matrices Q and Z, respectively.
C
...
C             the transformed matrix C*diag(Zc,I).
C
C     LDC     INTEGER
...
C     REVISIONS
C
C     V. Sima, Feb. 2012, April 2012, June 2012.
C
C     KEYWORDS
C
...
C     .. External Subroutines ..
     ...
     $                   DLASET, DORGQR, DORMQR, DROT, DSWAP, XERBLA
...
C
     ...
     $                   B, LDB, DWORK, -1, INFO )
            MAXWRK = MAX( MAXWRK, INT( DWORK(1) ) )
            IF( ILQ ) THEN
               CALL DORGQR( L, N1, N1, Q, LDQ, DWORK, DWORK, -1, INFO )
               MAXWRK = MAX( MAXWRK, INT( DWORK(1) ) )
            END IF
         ELSE IF( LQUERY ) THEN
            MAXWRK = MINWRK
...
C
   ...
   10       CONTINUE
     ...
     $                   B, LDB, DWORK(N1+1), LDWORK-N1, INFO )
            MAXWRK = MAX( MAXWRK, INT( DWORK(N1+1) )+N1 )
            IF( ILQ ) THEN
               CALL DLACPY( 'Lower', N1, N1, E, LDE, Q, LDQ )
               CALL DORGQR( L, N1, N1, Q, LDQ, DWORK, DWORK(N1+1),
     $                      LDWORK-N1, INFO )
               MAXWRK = MAX( MAXWRK, INT( DWORK(N1+1) )+N1 )
            END IF
         END IF
