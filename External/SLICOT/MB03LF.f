...
C             LIWORK >= N/2 + 18,    if COMPQ = 'N' and COMPU = 'N';
C             LIWORK >= MAX( N + 48, 5*N/2 + 1 ), otherwise.
C
...
C     REVISIONS
C
C     V. Sima, Feb. 2011, Aug. 2011, Nov. 2011, Oct. 2012.
C     M. Voigt, Jan. 2012.
C
C     KEYWORDS
C
...
C                         w  = 3/2*N**2 + MAX(N, 48), if  COMPQ = 'N'
C                                                     and COMPU = 'N';
C                         w  =   3*N**2 + MAX(N, 48), otherwise;
C                         prefer larger.
