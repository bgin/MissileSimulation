...
C
     ...
     $                      DWORK, Q, LDQ, DWORK, -1, INFO )
            ...
            END IF
            K = MIN( L, N-1 )
            CALL DTZRZF( K, N, E, LDE, DWORK, DWORK, -1, INFO )
            WRKOPT = MAX( WRKOPT, LN + INT( DWORK(1) ) )
            CALL DORMRZ( 'Right', 'Transpose', L, N, K, N, E, LDE,
     $                   DWORK, A, LDA, DWORK, -1, INFO )
            WRKOPT = MAX( WRKOPT, N + INT( DWORK(1) ) )
            IF( WITHC ) THEN
               CALL DORMRZ( 'Right', 'Transpose', P, N, K, N, E, LDE,
     $                      DWORK, C, LDC, DWORK, -1, INFO )
               WRKOPT = MAX( WRKOPT, N + INT( DWORK(1) ) )
            END IF
            IF( ILZ ) THEN
               CALL DORMRZ( 'Right', 'Transpose', N, N, K, N, E, LDE,
